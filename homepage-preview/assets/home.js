  ...
  $$('[data-open-dialog]').forEach(button => button.addEventListener('click', () => {
    switch(button.dataset.openDialog) {
      case 'feed-info':
        ...
        break;
      case 'newsletter':
        if(safeUrl(data.links.newsletterUrl)){window.open(data.links.newsletterUrl,'_blank','noopener,noreferrer');return;}
        openDialog('STAY CONNECTED','PSYWERX newsletter',[
          node('p','The existing newsletter will remain part of the new site, with a quiet signup link rather than an intrusive popup.'),
          node('p','Preview only: the verified mailing-list signup URL still needs to be connected. This preview does not collect or submit email addresses.')
        ]);
        break;
      case 'linkedin':
        if(safeUrl(data.links.linkedinUrl)){window.open(data.links.linkedinUrl,'_blank','noopener,noreferrer');return;}
        openDialog('STAY CONNECTED','PSYWERX on LinkedIn',[
          node('p','The new footer reserves this place for the PSYWERX company page.'),
          node('p','Preview only: connect the existing verified company-page URL before launch. No destination has been guessed.')
        ]);
        break;
    }
  }));

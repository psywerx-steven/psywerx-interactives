"""Deterministic, candidate-only Social Layer V2 inventory and protection gates."""
...
BASE_COMMIT = "bb9336b9b1cf0bcb110cbf39da93a67d38c64cea"
PROGRAM_ID = "AUD-SOCIAL-LAYER-AE-V1-20260922-001"
PROTECTED = (
...
def baseline() -> dict:
    ...
    raw = {r["id"]: r for r in read(ROOT / "data/entities.json") if r["layer"] == "Social"}
    mechanical = {r["id"]: r for r in inventory["entities"]}
    families = [r for r in read(ROOT / "data/families.json")["families"] if r["layer"] == "Social"]
    ...
    return {"schemaVersion": "1.0.0", "programId": PROGRAM_ID,
        ...
        "mechanicalCounts": {"families": len(families),
            ...
            "networkStateBindings": 1}}


def validate_protection() -> None:
    expected = read(DATA / "protected-baseline.json")["productionHashes"]
    assert hashes() == expected, "Production science changed after Social baseline"


def init() -> None:
    if (DATA / "baseline.json").exists():
        raise ValueError("Social baseline already frozen")
    value = baseline()
    ...
    write_doc(DOCS / "SOCIAL_LAYER_PLAN.md", f"# Social Layer Scale-Up V2 plan\n\n{notice}\n\nProgram `{PROGRAM_ID}` freezes main `{BASE_COMMIT}`. All 12 Families receive existing-edge, Driver, RDS, evidence, Network State and Actions & Events coverage. Deep research follows exact preliminary signal only. No production proposition, source, ontology, architecture or lifecycle state changes.\n")
    write_doc(DOCS / "SOCIAL_LAYER_PROGRESS.md", "# Social Layer progress\n\n" + notice + "\n\n| Family | Stage |\n|---|---|\n" + "\n".join(f"| {r['id']} | BASELINE |" for r in value["families"]))
    write_doc(DOCS / "SOCIAL_LAYER_BASELINE.md", f"# Social Layer frozen baseline\n\n{notice}\n\nProgram `{PROGRAM_ID}`; source main `{BASE_COMMIT}`.\n\n| Measure | Count |\n|---|---:|\n| Families | {c['families']} |\n| Drivers | {c['drivers']} |\n| RDS | {c['rds']} |\n| Entities | {c['entities']} |\n| Incident Relationships | {c['incidentRelationships']} |\n| Causal propositions | {c['causalRelationships']} |\n| Causal isolates | {c['causalIsolates']} |\n| RDS causal sources | {len(c['rdsCausalSources'])} |\n| V1-incomplete causal | {c['incidentCausalV1Incomplete']} |\n| Blocked entities | {c['blockedEntities']} |\n| Network State bindings | 1 |\n\nCausal scope: `{c['causalScope']}`.\n")

...
def validate_guides(corpus: Corpus, payload: dict[str, Any]) -> None:
    require(set(payload) == {"schemaVersion", "membershipMethod", "unmappedAllowed", "guides"}, "Guide payload is not allowlisted")
    require(payload.get("schemaVersion") == "1.1", "Citation-only guide schema 1.1 required")
...
def render_episode(corpus: Corpus, e: dict[str, Any], order: int) -> str:
...
def render_guide(corpus: Corpus, g: dict[str, Any]) -> str:
...
def compile_outputs(root: Path, payload: dict[str, Any] | None = None) -> tuple[dict[Path, bytes], dict[str, Any]]:
    corpus = Corpus(root)
    ...
    directory = []
    for g in guides:
        out[SITE / g['slug'] / 'index.html'] = render_guide(corpus,g).encode('utf-8')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Topic Guides: {report['guideCount']} guides, {report['selectedSourceCards']} selected source cards; citation-only episode cards; canonical sources unchanged.")
        return 0
